...
public class results:UIViewController{
    ...
    private func buttons(){
        ...
        if isWin {
            buttonTexts = ["RESTART","MENU","NEXT"]
        }
        else{
            buttonTexts = ["RESTART","MENU"]
        }
    ...
    @objc private func OnButonPressed(_ sender: UIButton){
        ...
        case 2:
            let vc = game()
            data.SetLevel(value: data.GetLevel() + 1)
            vc.leveValue = data.GetLevel()
            if data.GetLevel() > data.GetMaxLevel(){
                data.SetMaxLevel(value: data.GetLevel())
            }
            data.SetPlayerCoins(value: 2500)
            openController(viewController: vc)
            break
        default:
            break
